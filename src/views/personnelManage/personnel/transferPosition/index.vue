<template>
  <div class="assistant">
    <div class="top-form">
      员工号搜索:
      <el-input v-model="gh" placeholder="请输入员工号" clearable @clear="getAllTransPosApp" class="top-input">
      </el-input>
    ...
    <div>
      <el-table :data="tableData" height="calc(100vh - 235px)" fit
        ...
        <el-table-column prop="xm" fixed label="申请人" min-width="100">
        </el-table-column>
        <el-table-column prop="gh" label="员工号" min-width="90">
        </el-table-column>
        <el-table-column prop="sqrq" label="申请日期" min-width="110">
        </el-table-column>
        <el-table-column prop="rzrq" label="入职日期" min-width="110">
        </el-table-column>
        <el-table-column prop="tzhbm" label="调整后部门" min-width="100">
        </el-table-column>
        <el-table-column prop="tzhzw" label="调整后职位" min-width="100">
        </el-table-column>
        <el-table-column prop="tzhzj" label="调整后职级" min-width="100">
        </el-table-column>
        <el-table-column prop="jjr" label="交接人" min-width="100">
        </el-table-column>
...
import {
  addTransPosApp,
  inquireTransPosApp,
  inquireOnlyTransPosApp,
  editTransPosApp,
  deleteTransPosApp
} from '@/api/StaffManagement/transferPos.js'
...
export default {
  ...
  created() {
    this.getAllTransPosApp()
  },
  methods: {
    // 获取所有职位信息
    getAllTransPosApp() {
      inquireTransPosApp().then((res) => {
        if (res.code == 200) {
    ...
    inquirer() {
      if (!this.gh) {
        ...
        inquireOnlyTransPosApp(this.gh).then((res) => {
